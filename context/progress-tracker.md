...
- Installed `@trigger.dev/sdk`, `@trigger.dev/build`, and the local `trigger.dev` CLI package at matching versions.
- Added `trigger.config.ts` with the root `trigger/` task directory, retry defaults, build extension placeholder, and one-hour task max duration.
...
- `npm install @trigger.dev/sdk` initially failed in the sandbox because the npm registry could not be resolved, then passed on an escalated rerun.
- `npm install --save-dev @trigger.dev/build trigger.dev` initially failed in the sandbox because the npm registry could not be resolved, then passed on an escalated rerun.
